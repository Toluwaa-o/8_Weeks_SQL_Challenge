...
SELECT
  customer_id,
  COUNT(men.product_name) as Ord_Amt
FROM
  sales sal
  JOIN menu men ON sal.product_id = men.product_id
...
FROM
  ...
  RIGHT JOIN (
    SELECT
      customer_id,
      MAX(Amt) AS TimesPurchased
    FROM
      ProdAmounts
    GROUP BY
      customer_id
  ) Tbl ON prod.Amt = Tbl.TimesPurchased
  AND prod.customer_id = Tbl.customer_id
...
FROM
  ...
  JOIN (
    ...
    GROUP BY
      customer_id
  ) subTable ON sal.order_date = subTable.order_date
  AND sal.customer_id = subTable.customer_id
  JOIN menu men ON sal.product_id = men.product_id
